...
import ProductGenuine from "./ProductGenuine";

function ProductFilter({data}:{data:any}) {
  const [sorting, setSorting] = useState(false);
  const size = useWindowSize();
  const isMobile: boolean = size.width < 640;
  return (
    <>
      <Box className="bg-white mb-5 p-3 rounded-lg">
      <h1 className="font-medium mb-2 sm:hidden">Tất cả sản phẩm</h1>
      ...
      <div className="sm:p-5">
        <ProductSuggest category_id={data.id} />
        {/* <ProductGenuine category_id={data.id} genuine={1}/> */}
      </div>
    </>
  );
}
